...
sys.path.insert(0, "./dino_patch")
from patch_embedding import load_dinov3_model

def extract_masks_from_image_batch(images, masks_batch, scores_batch, box_coords_batch, input_labels_batch=None, borders=True,masking=True):
    """
    ...
    if masking:
        ...
        for image, box_coords in zip(images, box_coords_batch):
            crops = []
            if isinstance(image, np.ndarray):
                image_pil = Image.fromarray(image)
            else:
...
def detect_segment_crop_pipeline_batch(
    ...
    coco_json_path = f"nms_boxes_coco.json" 
    
    batch_size = kwargs.get("batch_size", 4)
    print("Using batch size:", batch_size)
    # Step 1: Detect objects (batched)
    if kwargs.get("detection",True):
        batch_boxes, batch_labels, batch_scores = owl_predict_batch(images=image_paths, texts=text_prompts, batch_size=batch_size)
    ...
    # Step 2: Save detections as coco per run 
    if save_coco:
        save_coco_json(image_paths, nms_boxes_batch, nms_labels_batch, nms_scores_batch, coco_json_path)
    nms_boxes_batch = [boxes for boxes in nms_boxes_batch]
    nms_labels_batch = [labels for labels in nms_labels_batch]
    nms_scores_batch = [scores for scores in nms_scores_batch]
    
     

    # prep crops to be processed, even if not doing segmentation within them 
    ...
    #print(f"Extracted {sum(len(crops) for crops in crops_batch)} crops for {len(image_paths)} images.")

     # Step 9: Visualization of crops (optional, batched)
